from django.contrib import admin
from django.utils.html import format_html

from .models import SpeakersModel


@admin.register(SpeakersModel)
class SpeakersAdmin(admin.ModelAdmin):
    """Админка для управления спикерами"""

    list_display = ["image_preview", "name", "position", "order", "created_at"]
    list_filter = ["created_at"]
    search_fields = ["name", "position"]
    readonly_fields = ["image_preview", "created_at"]
    ordering = ["order", "name"]

    fieldsets = (
        (
            "Основная информация",
            {"fields": ("name", "position", "image", "image_preview")},
        ),
        ("Настройки", {"fields": ("order", "created_at")}),
    )

    def image_preview(self, obj):
        """Превью изображения в админке"""
        if obj.image:
            return format_html(
                '<img src="{}" style="max-width: 100px; max-height: 100px; object-fit: cover; border-radius: 4px;" />',
                obj.image.url,
            )
        return "Нет изображения"

    image_preview.short_description = "Превью"
